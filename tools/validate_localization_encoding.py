...
class LocalizationValidator:
        ...
        try:
            ...
            if not has_bom:
                if self.fix_mode:
                    ...
                    self.errors.append(
                        f"NG {file_path}: Missing UTF-8 BOM (required for HOI4 localization)"
                    )
                    return False
            ...
            try:
                with open(file_path, "r", encoding="utf-8-sig") as f:
                    f.read()
                self.valid.append(f"OK {file_path}: Correct UTF-8 with BOM encoding")
                return True

            except UnicodeDecodeError as e:
                self.errors.append(f"NG {file_path}: Invalid UTF-8 encoding: {e}")
                return False

        except Exception as e:
            self.errors.append(f"NG {file_path}: Unexpected error: {e}")
            return False
    ...
    def _fix_file(self, file_path: Path, content: bytes) -> bool:
        ...
        try:
            ...
            self.errors.append(f"NG {file_path}: Cannot fix - invalid UTF-8: {e}")
            return False
        except Exception as e:
            self.errors.append(f"NG {file_path}: Error while fixing: {e}")
            return False
    ...
    def validate_files(self, files: List[Path]) -> bool:
        ...
        for file_path in files:
            if not file_path.exists():
                self.errors.append(f"NG {file_path}: File not found")
                all_valid = False
                continue
...
Examples:
  ...
  python validate_localization_encoding.py --fix localisation/english/*.yml
    ...
    if args.files:
        ...
        if not files:
            print("NG No English localization files found", file=sys.stderr)
            print(

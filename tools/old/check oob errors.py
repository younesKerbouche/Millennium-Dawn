        ...
        for line in content:
            if not line.startswith("#") or line.startswith(
                ...
                if startDate == 1:
                    ...
                    if openBrace == 1:
                        hasTech = re.search(
                            r"[ \t]+([A-Za-z0-9_\-]+)\s?=\s?1", line, re.M | re.I
                        )  # If it's a tag
                        if hasTech:
                            # updatedTags[tagPos].append([])
            ...
            if not line.startswith("#") or line.startswith(
                ...
                if startDate == 1:
                    ...
                    if openBrace == 1 and ("name" in line or "type" in line):
                        variantName = re.search(
                            r"name\s?=\s?\"(.*)\"", line, re.M | re.I
                        )  # If it's a tag
                        variantType = re.search(
                            r"type\s?=\s?(.*)", line, re.M | re.I
                        )  # If it's a tag
        ...
        for line in content:
            if not line.startswith("#") or line.startswith(
                ...
                if "ship" in line and "#ship" not in line:
                    ...
                    startReading = 1
                if (
                    "add_equipment_to_stockpile" in line
                    and "#add_equipment_to_stockpile" not in line
                ):
                    stockpile = 1
                    startReading = 1
                if (
                    "add_equipment_production" in line
                    and "#add_equipment_production" not in line
                ):
                    production = 1
                    startReading = 1
                if startReading == 1:
                    ...
                    if "}" in line:
                        openBrace -= 1
                    if ship == 1:
                        hasEquipment = re.search(
                            r"equipment\s?=\s?{\s?([A-Za-z0-9_\-]+)\s?=",
                            line,
                            re.M | re.I,
                        )  # If it's a tag
                        hasVersion = re.search(
                            r"version_name\s?=\s?\"(.*)\"", line, re.M | re.I
                        )  # If it's a tag
                        hasCreator = re.search(
                            r"creator\s?=\s?([A-Z]{3})", line, re.M | re.I
                        )  # If it's a tag
                        hasOwner = re.search(
                            r"owner\s?=\s?([A-Z]{3})", line, re.M | re.I
                        )  # If it's a tag
                        if hasCreator:
                    ...
                    if stockpile == 1:  # change to 1, set at 5 as I need to fix techs
                        hasEquipment = re.search(
                            r"type\s?=\s?([A-Za-z0-9_\-]+)", line, re.M | re.I
                        )  # If it's a tag
                        hasVersion = re.search(
                            r"version_name\s?=\s?\"(.*)\"", line, re.M | re.I
                        )  # If it's a tag
                        hasCreator = re.search(
                            r"producer\s?=\s?([A-Z]{3})", line, re.M | re.I
                        )  # If it's a tag
                        hasOwner = re.search(
                            r"([A-Z]{3})_", fileName, re.M | re.I
                        )  # If it's a tag
                        if hasCreator:
                            creator = hasCreator.group(1)
                        if hasVersion:
                    ...
                    if creator and version_name and equipment_name:
                        foundVar, foundTech = check_variant(
                            creator, version_name, equipment_name, startDate, tags
                        )

                        if (
                            version_name != "yes"
                            and foundVar == 0
                            and (ship == 1 or production == 1)
                        ):
                            # print(startDate)
                            print(
                                "ERROR: "
                        ...
                        if foundTech == 0:
                            ...
                            print(
                                ...
                                + " but "
                                + creator
                                + " doesn't have this tech unlocked"
                            )
...
def main():
    ...
    for root, dirnames, filenames in os.walk(
        rootDir + "/" + "history" + "/units" + "/"
    ):
        for filename in fnmatch.filter(filenames, "*.txt"):
            analyzeMyVariants(tags, os.path.join(root, filename), filename)

    print(
        "The script took {0} second!".format(time.time() - startTime)
        + " therea are a total of: "
        + str(totalErrors)
        + " errors."
    )

...
def testFile(file_path):
    ...
    try:
        ...
        print(
            "\033[31mCan't read '{}'. Skipping file and moving on...\033[0m".format(
                file_path, linesRead + 1
            )
        )
        filetoTest.close()
        return False
...
def main():
    ...
    parser.add_argument(
        "--redpanda", action="store_true", help="show a secret and exit"
    )
    parser.add_argument(
        "-v",
        "--version",
        action="version",
        version="Build script version {}.".format(__version__),
    )
    ...
    if PROGRAM == "relations":
        if not len(INPUTLINES) < 1:
            input(
                "WARNING! this will write lines inside of a exising files. Press enter to start..."
            )
            ...
            for nation in nationFileList:
                ...
                if (nation[:3] in WHITELIST) and (not nation[:3] in BLACKLIST):
                    ...
                    if testPassed:
                        print(
                            "Looking for default relation position...".format(
                                nation[:3]
                            )
                        )
                        ...
                        if not ideasTopics == []:
                            ...
                            for ideaTopic in ideasTopics:
                                ...
                                onNewline += run
                                run += 1
                                if (YEARFILTER == 2011 and run == 1) or (
                                    YEARFILTER == 2017 and run >= 2
                                ):
                            ...
                            print(
                                "Could not find default location inserting on last line(s)..."
                            )
            ...
            sys.exit(
                "MDTOOL Relations: error: Relations program require --stringinsert argument..."
            )
